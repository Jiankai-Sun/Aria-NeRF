...
def parse_args():
    ...
    parser.add_argument(
        "--split_nth", default=8, type=int, help="select every n-th frame as validation and every other n-th frame as test frame."
    )
...
def run(args):
    print('Downscaling factor: {}'.format(args.scale))
    colmap_model_dir = os.path.join(args.colmap_dir, 'sparse/0')
    colmap = load_colmap(colmap_model_dir)
    meta = calc_meta(colmap, split_nth=args.split_nth, image_downscale=args.scale)
