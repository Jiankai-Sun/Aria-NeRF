...
class NeuralDiffSystem(pytorch_lightning.LightningModule):
    def __init__(self, hparams, train_dataset=None, val_dataset=None):
        ...
        self.embedding_t = model.LREEmbedding(
            N=hparams.N_vocab, D=hparams.N_tau, K=hparams.lowpass_K, device=self.device_name
        )
    ...
    def validation_step(self, batch, batch_nb, is_debug=False):
        ...
        log["val_psnr"] = psnr_
        if self.hparams.use_lpips:
            # print('results[rgb_fine].max(), rgbs.max(), results[rgb_fine].min(), rgbs.min(): ',
            #       results["rgb_fine"].max(), rgbs.max(), results["rgb_fine"].min(), rgbs.min())
            img1 = results["rgb_fine"].reshape(1, self.train_dataset.img_h, self.train_dataset.img_w, 3).permute(0, 3, 1, 2)
            img2 = rgbs.reshape(1, self.train_dataset.img_h, self.train_dataset.img_w, 3).permute(0, 3, 1, 2)
            print('img1.shape, img2.shape: ', img1.shape, img2.shape)
            self.lpips.update(img1, img2)
            lpips_value = self.lpips.compute()
            log["val_lpips"] = lpips_value

        if self.hparams.use_ssim:
            # print('results[rgb_fine].max(), rgbs.max(), results[rgb_fine].min(), rgbs.min(): ',
            #       results["rgb_fine"].max(), rgbs.max(), results["rgb_fine"].min(), rgbs.min())
            ssim_value = ssim(results["rgb_fine"].cpu().numpy(), rgbs.cpu().numpy())
            log["val_ssim"] = torch.tensor([ssim_value])
    ...
    def validation_epoch_end(self, outputs):
        ...
        if self.hparams.use_lpips:
            ...
            self.log("val/lpips", mean_lpips, prog_bar=True)
        if self.hparams.use_ssim:
