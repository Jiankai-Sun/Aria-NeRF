...
lpips = metrics.LearnedPerceptualImagePatchSimilarity(normalize=True)
...
def evaluate_sample(
    ...
    if pose is None:
        try:
            ...
            print('{} does not exist! Skip'.format(sample_id))
            return None
    else:
    ...
    if use_lpips:
        img1 = img_pred.reshape(1, ds.img_h, ds.img_w, 3).permute(0, 3, 1, 2)
        img2 = img_gt.reshape(1, ds.img_h, ds.img_w, 3).permute(0, 3, 1, 2)
        # print('img1.shape, img2.shape: ', img1.shape, img2.shape)
        # mg1.shape, img2.shape:  torch.Size([1, 3, 180, 193]) torch.Size([1, 3, 180, 193])
        img1 = torch.clip(img1, 0, 1)
        img2 = torch.clip(img2, 0, 1)
        lpips.update(img1, img2)
        lpips_value = lpips.compute()
    if use_ssim:
        ssim_value = metrics.ssim(img_pred.cpu().numpy(), img_gt.cpu().numpy())
...
def evaluate(
    ...
    for i, sample_id in utils.tqdm(enumerate(image_ids), total=len(image_ids)):
        ...
        try:
            ...
            print('mask_targ and im_masked do not exist. Skip.')
            mask_targ, im_masked = None, None
        ...
        if use_lpips:
            results["lpips"].append(out["lpips"])
        if use_ssim:
            results["ssim"].append(out["ssim"])
        if mask_targ is None:
            mask_targ = mask_pred
            results["avgpre"].append(0.)
        else:
    ...
    if use_lpips:
        print('LPIPS: {}'.format(results['metrics']['lpips']))
    if use_ssim:
        print('SSIM: {}'.format(results['metrics']['ssim']))
    if save:
        print('Masks saved to {}'.format(save_dir))
    return results
